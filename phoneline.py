...
class PhoneLine:
    ...
    def make_call(self, call: Call) -> None:
        ...
        # add a call to the numbers calling history and bill the call
        self.callhistory.register_outgoing_call(call)

        # see if we are starting a new month or not
        date = call.time
        bill_tuple = (date.month, date.year)
        if bill_tuple not in self.bills:
            self.new_month(bill_tuple[0], bill_tuple[1])

        # now bill the call
        self.contract.bill_call(call)
        self.bills[bill_tuple[0], bill_tuple[1]] = self.contract.bill
    ...
    def receive_call(self, call: Call) -> None:
        ...
        # setting a new month if there is no bill for the
