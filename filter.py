...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        for person in customers:
            all_ids.append(person.get_id())

        try:
            if isinstance(int(filter_string), int) is not True:
                raise TypeError
            if int(filter_string) not in all_ids:
                raise IndexError
        except Exception:
            return data

        calls_made = \
            customers[all_ids.index(int(filter_string))].get_history()[0]
        for call in data:
            if call in calls_made:
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            if filter_string[0] not in cases or \
                    isinstance(int(filter_string[1:]), int) is not True or int(
                    filter_string[1:]) > 999:
                raise TypeError
        except Exception:
            return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        try:
            if (len(coords) != 4) \
                    or (isinstance(float(coords[0]), float) is not True) \
                    or (isinstance(float(coords[1]), float) is not True) or \
                    (isinstance(float(coords[2]), float) is not True) or (
                    isinstance(float(coords[3]), float) is not True):
                raise TypeError
        except Exception:
            return data
        ...
        for call in data:
            if ((coords[0] <= call.src_loc[0] <= coords[2])
                and (coords[1] <= call.src_loc[1] <= coords[3])) \
                    or ((coords[0] <= call.dst_loc[0] <= coords[2])
                        and (coords[1] <= call.dst_loc[1] <= coords[3])):
                filtered_calls.append(call)
