...
class CallHistory:
    ...
    def register_outgoing_call(self, call: Call) -> None:
        ...
        """
        date = call.time
        date_tuple = (date.month, date.year)
        if date_tuple not in self.outgoing_calls:
            self.outgoing_calls[date_tuple] = [call]
        else:
            self.outgoing_calls[date_tuple].append(call)
    ...
    def register_incoming_call(self, call: Call) -> None:
        ...
        date = call.time
        date_tuple = (date.month, date.year)
        if date_tuple not in self.incoming_calls:
            self.incoming_calls[date_tuple] = [call]
        else:
            self.incoming_calls[date_tuple].append(call)
