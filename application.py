...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        # the following code checks if it is the next month
        # and if it is, it runs every customer through the new_month function
        curr_date = datetime.datetime.strptime(event_data['time'],
                                               "%Y-%m-%d %H:%M:%S")
        curr_month = curr_date.month
        curr_year = curr_date.year
        if curr_month != billing_month:
            billing_month = curr_month
            billing_year = curr_year
            new_month(customer_list, billing_month, billing_year)

        # the following code creates a new outgoing call, and call recieved
        if event_data['type'] == 'call':
            new_call = Call(event_data['src_number'], event_data['dst_number'],
                            curr_date, event_data['duration'],
                            event_data['src_loc'], event_data['dst_loc'])

            calling_customer = find_customer_by_number(event_data['src_number'],
                                                       customer_list)
            receiving_customer = \
                find_customer_by_number(event_data['dst_number'], customer_list)

            calling_customer.make_call(new_call)
            receiving_customer.receive_call(new_call)
